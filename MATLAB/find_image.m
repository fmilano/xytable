...
function [imagen,escala,num_imagenes] = find_image(titulo,ventanaX,ventanaY,pos_im,true_size,hw)

    pos_im(find (pos_im<0)) = 0;
    ...
    for fila = primer_fila:ultima_fila
        ...
        primer_imagen_index = find(pos_im(fila,2:end,1)>=ventanaX(1),1,'first');
        ultima_imagen_index = find(pos_im(fila,2:end,1)>=ventanaX(2),1,'first');
        ...
        for im_index = primer_imagen_index:ultima_imagen_index
            ...
            if (strcmp(titulo(1:6),'Final_'))
                im_aux = imread(['/home/axel/Desktop/XYTableAcData/' titulo(7:end) '/' titulo '_0' num2str(pos_im(fila,1+im_index,2)) '.png']); 
            else
                im_aux = imread(['/home/axel/Desktop/XYTableAcData/' titulo '/' titulo '_0' num2str(pos_im(fila,1+im_index,2)) '.png']);
            end
            ...
            try 
                ...
                disp('Error: Index exceeds matrix dimensions');
            end
            if (isempty(porcion))
                disp('Error');
                imagen = 0;
        ...
        % Se actualiza la waitbar
        if (hw ~= 0)
            waitbar(fila/ultima_fila,hw);
        end
